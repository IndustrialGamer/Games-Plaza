...
public class TimeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("tempo")) {
            ...
            if (p.hasPermission("tempo.time.change")) {
                if (args.length == 0) {
                    p.sendMessage(ChatColor.BLUE + "Tempo> " + ChatColor.RED + "Use /tempo <t/h> <tm/c/s/d/e/a/n/md>!");
                    return true;
                }
                if (args.length > 4) {
                ...
                if (args[0].equalsIgnoreCase("h")) {
                    ...
                    if (args[1].equalsIgnoreCase("n")) {
                        world.setTime(14000);
                        p.sendMessage(ChatColor.BLUE + "Tempo> " + ChatColor.GREEN + "O tempo foi alterado de " + ChatColor.DARK_PURPLE + world.getTime() + " " + ChatColor.GREEN + "para NOITE (14000) com sucesso!");
                    }
                }

                if (args[0].equalsIgnoreCase("t")) {
                    if (args[1].equalsIgnoreCase("tm")) {
